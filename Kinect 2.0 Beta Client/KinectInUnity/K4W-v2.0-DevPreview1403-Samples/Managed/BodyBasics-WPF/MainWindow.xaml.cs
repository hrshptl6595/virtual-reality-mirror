                                ...
                                if (body.IsTracked)
                                {
                                    ...
                                    foreach (JointType jointType in joints.Keys)
                                    {
                                        ...
                                        if (jointType == JointType.HandRight)
                                            sURL += "HandRight";
                                        sURL += ":";
                                        sURL += joints[jointType].Position.X.ToString() + "," + joints[jointType].Position.Y.ToString() + "," + joints[jointType].Position.Z.ToString();
                                        sURL += "\n";
                                    }
                                    ...
                                    if (true)//(random.Next(0, 40) == 4)
                                    {
                                        using (var client = new WebClient())
                                        {
                                            var data = new NameValueCollection();
                                            string serverAddress = "http://127.0.0.1:3000/kinect";
